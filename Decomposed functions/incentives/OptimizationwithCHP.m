...
cf=3600 * (8314 / 17.7) * 273.15 / 101325 / 48000;
...
%     Constr=[Constr OUTPUT{i}(1,1:(end-1)) - OUTPUT{i}(1,2:end) <= Machines{i,6}(1)*timestep.*Z(i,2:end) + (Z(i,1:(end-1))-Z(i,2:end)).*(OUTPUT{i}(1,1:(end-1))]; 
    ...
    %CHP constrant
    if ismember(Machines{i,1},'ICECHP')
        Constr=[Constr INPUT{i}(:)*cf <= OUTPUT{i}(1,:)'*0.22];
    end
    
    if ismember(Machines{i,1},'ICE')
        fuelsave=sdpvar(1,ntimes);
        Constr=[Constr fuelsave == OUTPUT{i}(1,:)*0.22/cf];
    end
    ...
    for i=1:Nmachines
        ...
        for h=1:numel(Machines{i,2})
            if isequal(Fuels{j,1},Machines{i,2}(h))
                l=l+1;
                if ismember(Machines{i,1},'ICE')
                    Constr=[Constr fuelcons{j}(l,:)==INPUT{i}(h,:)+delta(i,:).*SUcosts(i)-fuelsave.*(1-Fuels{2,2}'./Fuels{1,2}')];
                else
...
Outs={INPUT{:} OUTPUT{:} STORAGEcharge STORAGEpower NETWORKbought NETWORKsold Diss slacks Zext(:,advance:(advance+histdepth-1)) fuelusage delta};

ops = sdpsettings('solver','gurobi','gurobi.MIPGap',0.01,'verbose',3);
