...
Diss=sdpvar(Noutputs,ntimes,'full');                                
Constr=[Constr Diss>=0];
Constr=[Constr Diss<=repmat(maxdiss',1,ntimes)];            %POTENTIAL CAUSE FOR INFEASIBILITY
...
%THERE MIGHT BE A SMARTER WAY OF DOING THIS, WITHOUT AUX VARIABLES --> WOULD
...
cont1=cellfun(@(x) x',OUTPUT,'UniformOutput',false);
cont1=[cont1{1:4}]';
Constr = [Constr PresICE <= cont1 + Machines{i,6}(1)];

%PV / Demand worst cases
PVres=sdpvar(2,ntimes);                 %Tuned Outside
...
PresBESS=sdpvar(2,ntimes);
Constr = [Constr PresBESS <= repmat(cell2mat(Storages(:,5)),1,ntimes)];
Constr = [Constr PresBESS <= (STORAGEcharge - repmat([Storages{:,2}]'.*[Storages{:,9}]'./100,1,ntimes)).*repmat(cell2mat(Storages(:,6)),1,ntimes)./(repmat(timestep',2,1))];

%Inverter equivalent contribution
PresInv=sdpvar(2,ntimes);
Constr = [Constr PresInv <= (PVres + PresBESS)*0.97];                       %FIXED INVERTER EFFICIENCY
Constr = [Constr PresInv <= repmat([125*0.97;125*0.97],1,ntimes)];          %FIXED INVERTER SIZES

%Actual reserve contraint
Constr = [Constr sum(PresInv,1)+sum(PresBESS,1) >= Dres];
