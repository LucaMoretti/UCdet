...
Filename='caso macchina intermediaria con storage.xlsm';
...
gap=0.1;        %half bandwith of uncertainty on forecast referred to current demand value ((per ora relativo, si può mettere un valore di qualche altro tipo))
...
for j=1:Noutputs
    ...
    if store(j)==1&&net(j)==1
        Constr=[Constr [netprod(j,:) + STORAGEpower(h,:) + NETWORKbought(k,:) - NETWORKsold(k,:) == D{2}(j,:)+Diss(j,:)]];
    elseif store(j)==1&&net(j)==0
        Constr=[Constr [netprod(j,:) + STORAGEpower(h,:) == D{2}(j,:)+Diss(j,:)]];
    elseif store(j)==0&&net(j)==1
        Constr=[Constr [netprod(j,:) + NETWORKbought(k,:) - NETWORKsold(k,:) == D{2}(j,:)+Diss(j,:)]];
    else
        Constr=[Constr [netprod(j,:) == D{2}(j,:)+Diss(j,:)]];
    end
end
...
for i=1:Noutputs
figure()
title(strcat(Outputs{i},' Demand / Production profiles caso true'))
xlabel('Timestep')
ylabel('Power [kW]')
hold on
...
for i=1:Nnetworks
    if sum(value(NETWORKbought(i,:)))>0
        cont=cont+1;
        tags{cont}=strcat(Networks{1,1}, 'bought from network');
        costvals{cont}=value(NETWORKbought(i,:)').*Networks{i,4};
    end
    if sum(value(NETWORKsold(i,:)))>0
        can=can+1;
        cont=cont+1;
        tags{cont}=strcat(Networks{1,1}, 'sold on network');
